...
import { getClients, addClient, deleteClient } from '../../services/clientsService'
...
export default function ClientsList(){
  ...
  const add = async () => { if(!name) return alert('Enter name'); await addClient({ name }); setName(''); fetch() }
  const remove = async (id) => { if(!confirm('Delete?')) return; await deleteClient(id); fetch() }
