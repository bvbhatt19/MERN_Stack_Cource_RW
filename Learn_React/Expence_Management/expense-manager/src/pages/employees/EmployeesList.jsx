...
import { getEmployees, addEmployee, updateEmployee, deleteEmployee } from '../../services/employeesService'
...
export default function EmployeesList(){
  ...
  const add = async () => { if(!name) return alert('Enter name'); await addEmployee({ name }); setName(''); fetch() }
  const remove = async (id) => { if(!confirm('Delete?')) return; await deleteEmployee(id); fetch() }
